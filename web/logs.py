...
class SQLiteLogQueryEngine(AbstractLogQueryEngine):
    def __init__(self, log_file_path=None, log_data=None, batch_size=1000):
        ...
        # Define and register UDFs for regex
        def sqlite_regexp_basic(pattern, text):
            try:
                return 1 if re.search(pattern, text) else 0
            except re.error: # Invalid regex
                return 0 
        self.conn.create_function("REGEXP", 2, sqlite_regexp_basic)

        def sqlite_regexp_flags(pattern, text, flags_int):
            try:
                return 1 if re.search(pattern, text, flags_int) else 0
            except re.error: # Invalid regex
                return 0
        self.conn.create_function("REGEXP_FLAGS", 3, sqlite_regexp_flags)
    ...
    def count_occurrences(self, s, ignore_case=False, nick=None):
        ...
        # Regex condition
        # REGEXP_FLAGS UDF is defined in __init__
        sql_conditions.append('REGEXP_FLAGS(?, message, ?)')
        params.extend([s, re.IGNORECASE if ignore_case else 0])
        
        # Nick condition
        if nick:
            if nick in VALID_NICKS:
                nick_placeholders = ', '.join(['?'] * len(VALID_NICKS[nick]))
                sql_conditions.append(f'nick IN ({nick_placeholders})')
                params.extend(VALID_NICKS[nick])
            else: # Unknown nick
                return 0 
        
        sql = 'SELECT COUNT(*) FROM logs WHERE ' + ' AND '.join(sql_conditions)
        
        cursor = self.conn.cursor()
        try:
            # Validate regex pattern before sending to SQL to avoid UDF errors not caught by sqlite3.OperationalError
            # Though our UDFs now have try-except for re.error.
            try:
                re.compile(s, re.IGNORECASE if ignore_case else 0)
            except re.error:
                app.logger.warn(f"Invalid regex pattern provided to count_occurrences: {s}")
                return 0

            cursor.execute(sql, params)
            return cursor.fetchone()[0]
        except sqlite3.OperationalError as e:
            # This can happen if the regex is invalid for SQLite's REGEXP.
            app.logger.error(f"SQLite operational error: {e} with SQL: {sql} and params: {params}")
            return 0
